...
def acMain(version):
	...
	global TRACK_DETAILS, TRACK_SURFACES
	TRACK_DETAILS = trackDetails()
	TRACK_SURFACES = trackSurfaces()
	...
	def readFastLane(self):
		try:
			with open(self.detailFile, "rb") as buffer:
				header, self.length, u1, u2 = struct.unpack("4i", buffer.read(4 * 4))
...
class trackSurfaces:
	def __init__(self):
		...
		self.surfaceIniFile = "%s/data/surfaces.ini" % TRACK_DIRECTORY
		self.readSurface()
		
		self.modelsIniFile = ""
		if not TRACK_CONFIG == "":
			self.modelsIniFile = "content/tracks/%s/models_%s.ini" % (TRACK_NAME, TRACK_CONFIG)
		else:
			self.modelsIniFile = "%s/models.ini" % TRACK_DIRECTORY
		if os.path.isfile(self.modelsIniFile):
			self.readModels()
		else:
			self.readKn5("%s/%s.kn5" % (TRACK_DIRECTORY, TRACK_NAME))
	...
	def readKn5(self, filename):
		return
	
	def getNearMeshes(self, position):
		return
...
class renderer:
	...
	def render(self, delta):
		...
		if len(NAMETAG_HIGHER_CAR.mesh) >= 3 and NAMETAG_HIGHER_CAR.tagOffset > NAMETAG_HIGHER_CAR.mesh[2][1]:
		...
		for CAR in CARS:
			...
			if CAR.alive and len(CAR.mesh) == 3:
			...
			if CONFIG.nameTagVisible and CAR.alive and len(CAR.mesh) == 3 and not (not CONFIG.nameTagViewSelf and CAR.id == CAR_FOCUSED):
...
class backgroundWorker:
			...
			if CAR.alive and CAR.position.x != 0 and CAR.position.y != 0:
				...
				if DELTA_RAW_IN_RANGE and AREA > 10:
					...
					for i, point in enumerate(DELTA_RAW.points):
						...
						DELTA.append([point.x, point.y, ALPHA])
					CAR.mesh = DELTA
		
		if CONFIG.nameTagVisible:
			def sort(car):
				if len(car.mesh) == 0: return 0
				return car.mesh[2][1]
	...
	def calculateMap(self):
		...
		NEAR_NODE = int(TRACK_DETAILS.length * SELF_CAR.node) + NODE_STEP
		...
		NEAR_NODE_FORWARD = int(CONFIG.viewForward * PER)
		NEAR_NODE_BACKWARD = int(CONFIG.viewBackward * PER)
		...
		self.mesh_track_raw = []
		for index in range(RANGE_BACKWARD, RANGE_FORWARD, NODE_STEP):
			NODE = index
			if NODE < 0:
			...
			if QUAD_IN_RANGE:
				...
				for i, point in enumerate(QUAD_RAW.points):
					rawIndex = index if i < 2 else index - NODE_STEP
					DISTANCE_GAIN = 1 - min(1, abs(rawIndex - (TRACK_DETAILS.length * SELF_CAR.node)) / W)
					ALPHA = max(0, min(DISTANCE_GAIN, getAlphaByPoint(point)))
					QUAD.append([point.x, point.y, ALPHA])
				self.mesh_track_raw.append(QUAD)
